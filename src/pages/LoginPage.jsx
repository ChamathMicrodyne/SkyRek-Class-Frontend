      ...
      <div className="bg-black/40 w-full h-full absolute top-0 flex justify-center items-center">
        <div className="w-[414px] h-[400px] bg-black/5 border-2 border-white/20 backdrop-blur-md text-white rounded-xl p-8 flex flex-col items-center justify-center space-y-8">
          ...
          <div className="w-full flex items-center bg-white/10 border border-white/20 rounded-full px-4 py-2 space-x-3">
            <input type="email" placeholder="Email" className="bg-transparent text-white w-full outline-none placeholder-white" onChange={(e) => {setEmail(e.target.value)}} value={email} />
          ...
          <div className="w-full flex items-center bg-white/10 border border-white/20 rounded-full px-4 py-2 space-x-3">
            <input type={showPassword ? 'text' : 'password'} placeholder="Password" className="bg-transparent text-white w-full outline-none placeholder-white" onChange={(e) => {setPassword(e.target.value)}} value={password}/>
          ...
          <button type="submit" className="bg-white text-black font-semibold rounded-full w-full py-3 hover:bg-gray-300 transition duration-300" onClick={handleLogin}>
            Login
          </button>

          <p className="text-[13px] mt-[-15px] mb-[-20px] cursor-default">Don't have an account? <Link to="/signup" className="underline">Sign up</Link></p>
        </div>
      </div>
    </div>
  );
